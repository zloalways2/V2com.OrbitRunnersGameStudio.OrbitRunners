...
public class GameManager : MonoBehaviour
{
    ...
    public Slider healthSlider;  // Новый слайдер для здоровья
    public Slider scoreSlider;   // Новый слайдер для очков
    ...
    public Slider winHealthSlider;  // Слайдер для здоровья на панели победы
    public Slider winScoreSlider;   // Слайдер для очков на панели победы
    public Slider loseHealthSlider; // Слайдер для здоровья на панели поражения
    public Slider loseScoreSlider;  // Слайдер для очков на панели поражения
    ...
    public void UpdateScore(int amount)
{
    ...
    void UpdateHealthText()
    {
        ...
        healthSlider.value = health; // Обновляем слайдер здоровья
    }

    void UpdateScoreText()
    {
        scoreText.text = $"{score:D3}/{maxScore}";
        scoreSlider.value = score; // Обновляем слайдер очков
    }
...
void UpdateWinPanelText()
{
    ...
    if (winHealthSlider != null)
    {
        winHealthSlider.maxValue = isDoubleHPActive ? 200 : 100;
        winHealthSlider.value = health;
    }
    if (winScoreSlider != null)
    {
        winScoreSlider.maxValue = maxScore;
...
void UpdateLosePanelText()
{
    ...
    // Установка значений слайдеров
    if (loseHealthSlider != null)
    {
        loseHealthSlider.maxValue = isDoubleHPActive ? 200 : 100;
        loseHealthSlider.value = health;
    }
    if (loseScoreSlider != null)
    {
        loseScoreSlider.maxValue = maxScore;
